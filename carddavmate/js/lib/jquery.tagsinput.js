	...
	$.fn.addTag = function(value, options)
	{
		...
		this.each(function()
		{
			...
			if(options.unique)
			{
				...
				if(skipTag == true)
					$('#'+id+'_tag').addClass('not_valid');	//Marks fake input as not_valid to let styling it
			}
			else
				var skipTag = false;
...
//				markup = markup + '<input id="'+id+'_tag" type="text" value=""'+(settings.useNativePlaceholder==true ? ' placeholder="'+settings.defaultText+'" data-default=""' : ' data-default="'+settings.defaultText+'"')+' />';
			...
			if(settings.interactive)
			{
				...
				if(settings.autocomplete_url != undefined)
				{
					...
					if(jQuery.Autocompleter !== undefined)
					{
						...
						{
							if(data)
								$('#'+id).addTag(data[0] + "", {focus: true, unique: settings.unique, trimInput: settings.trimInput});
						});
					}
					else if(jQuery.ui.autocomplete !== undefined)
					{
						...
						{
							$(event.data.real_input).addTag(ui.item.value, {focus: true, unique: settings.unique, trimInput: settings.trimInput});
							return false;
						});
					}
					...
					$(data.fake_input).bind('blur', data, function(event)
					{
						...
						if($(event.data.fake_input).val()!='' && $(event.data.fake_input).val()!=d)
						{
								if((event.data.minChars <= $(event.data.fake_input).val().length) && (!event.data.maxChars || (event.data.maxChars >= $(event.data.fake_input).val().length)))
									$(event.data.real_input).addTag($(event.data.fake_input).val(), {focus: true, unique: settings.unique, trimInput: settings.trimInput});
	...
	$.fn.tagsInput.importTags = function(obj, val)
	{
		...
		for(var i=0; i<tags.length; i++)
			$(obj).addTag(tags[i], {focus: false, callback: true});
