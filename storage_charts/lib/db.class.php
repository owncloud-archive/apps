...
class OC_DLStCharts {
	...
	public static function update($used, $total) {
		...
		if($result) {
			...
			$query = OC_DB::prepare("INSERT INTO *PREFIX*dlstcharts (oc_uid,stc_month,stc_dayts,stc_used,stc_total) VALUES (?,?,?,?,?)");
			$query->execute(Array(OC_User::getUser(), date('Ym'), mktime(0,0,0), $used, $total));
		}
	}
	...
	public static function setUConfValue($key,$val) {
		...
		if(!is_null($conf)) {
			...
			$query = OC_DB::prepare("INSERT INTO *PREFIX*dlstcharts_uconf (oc_uid,uc_key,uc_val) VALUES (?,?,?)");
			$query->execute(Array(OC_User::getUser(), $key, $val));
		}
	}
