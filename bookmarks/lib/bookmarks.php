...
class OC_Bookmarks_Bookmarks {
	...
	if($filters == '*' || @$filters[0] == '*') {
	    // do nothing, show all bookmarks
	} elseif ($filterTagOnly) {
	...
	$result = $query->execute();
	// emit event
	OCP\Util::emitHook('OC_Bookmarks_Bookmarks', 'deleteBookmark', $id);
	// return
	return true;
    }
    ...
    /**
     * Return a bookmark in array form
     * @param int $id
     * @return array
     */
    public static function getBookmark($id) {
	// get the bookmark
	$query = OCP\DB::prepare("SELECT * FROM *PREFIX*bookmarks WHERE id = ? AND user_id = ?");
	$params = array($id, OCP\User::getUser());
	$result = $query->execute($params);
	// return
	if ($result->numRows() == 0) {
	    return null;
	...
	// emit event
	OCP\Util::emitHook('OC_Bookmarks_Bookmarks', 'editBookmark', $id);
    }
	...
	if ($b_id !== false) {
	    ...
	    // emit event
	    OCP\Util::emitHook('OC_Bookmarks_Bookmarks', 'addBookmark', $b_id);
	    // return
	    return $b_id;
	}
    }
