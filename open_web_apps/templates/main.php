...
if((remoteStorageOrigin == '') || (window.location == remoteStorageOrigin)) {
  document.getElementById('icons').innerHTML = 'You need to point a second origin to your server, so a subdomain, or a port other than '+window.location+'. Then go to Setting -> Admin -> open web apps and set the storage origin.';
} else {
  render();
}
</script>
