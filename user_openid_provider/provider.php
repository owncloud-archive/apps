...
if (OCP\User::isLoggedIn() and !$session->getLoggedInUser()) {
	$session->setLoggedInUser(OCP\Util::linkToAbsolute('user_openid_provider', 'provider.php').'/'.OCP\User::getUser());
}
...
if (isset($_GET['openid_action']) and $_GET['openid_action']=='login') {
	...
	$loginPage = OCP\Util::linkToAbsolute( '', 'index.php' ).'?redirect_url='
		.urlencode(OCP\Util::linkToAbsolute('user_openid_provider', 'provider.php')
				. $params);
