...
if( $task eq 'read' ){
	...
	foreach my $dir ( @dirs ){
		...
		foreach my $file ( @totranslate ){
			...
			print "    Reading $file\n";
			`xgettext --output="$output" $joinexisting $keywords --language=$language "$file" --add-comments=TRANSLATORS --from-code=UTF-8 --package-version="7.0.0" --package-name="ownCloud Apps" --msgid-bugs-address="translations\@owncloud.org"`;
		}
		chdir( $whereami );
	}
}
